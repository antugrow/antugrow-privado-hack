...
export function initInMemoryDataStorage({ contractAddress, rpcUrl }: { contractAddress: string; rpcUrl: string }): IDataStorage {
	...
	// conf.maxPriorityFeePerGas = '250000000000';
	// conf.maxFeePerGas = '250000000000';
	// conf.defaultGasLimit = 250000000000;
...
export async function initIdentityWallet(dataStorage: IDataStorage, credentialWallet: ICredentialWallet, keyStore: AbstractPrivateKeyStore): Promise<IIdentityWallet> {
	const bjjProvider = new BjjProvider(KmsKeyType.BabyJubJub, keyStore);
	const kms = new KMS();
	kms.registerKeyProvider(KmsKeyType.BabyJubJub, bjjProvider);

	const credentialStatusPublisherRegistry = new CredentialStatusPublisherRegistry();
	credentialStatusPublisherRegistry.register(CredentialStatusType.Iden3ReverseSparseMerkleTreeProof, new Iden3SmtRhsCredentialStatusPublisher());
...
export async function initInMemoryDataStorageAndWallets(config: { contractAddress: string; rpcUrl: string }) {
...
export async function initCredentialWallet(dataStorage: IDataStorage): Promise<CredentialWallet> {
	...
	resolvers.register(CredentialStatusType.Iden3OnchainSparseMerkleTreeProof2023, new OnChainResolver([defaultEthConnectionConfig]));
